...
def add_parser(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=[1.],
        help='same as SIMD devkit rate, but only change windows size')
    parser.add_argument(
        '--img-rate-thr',
        type=float,
        default=0.6,
...
def load_simd(img_dir, ann_dir=None, nproc=10):
    """Load SIMD dataset.
    ...
    print('Starting loading SIMD dataset information.')
    start_time = time.time()
    _load_func = partial(_load_simd_single, img_dir=img_dir, ann_dir=ann_dir)
    if nproc > 1:
        pool = Pool(nproc)
    ...
    end_time = time.time()
    print(f'Finishing loading SIMD, get {len(contents)} iamges,',
          f'using {end_time - start_time:.3f}s.')

    return contents


def _load_simd_single(imgfile, img_dir, ann_dir):
    """Load SIMD's single image.
    ...
    content = _load_simd_txt(txtfile)
...
def _load_simd_txt(txtfile):
    """Load SIMD's txt annotation.
...
def main():
    ...
    for img_dir, ann_dir in zip(args.img_dirs, args.ann_dirs):
        _infos = load_simd(img_dir=img_dir, ann_dir=ann_dir, nproc=args.nproc)

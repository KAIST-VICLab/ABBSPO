...
class ABBSPODetector(SingleStageDetector):
    """Implementation of `ABBSPO <https://openaccess.thecvf.com/content/CVPR2025/html/Lee_ABBSPO_Adaptive_Bounding_Box_Scaling_and_Symmetric_Prior_based_Orientation_CVPR_2025_paper.html>`_"""
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # attach corresponding img_metas (including crop_size) to each view
        for j, gt_instances in enumerate(
                batch_gt_instances + batch_gt_rot + batch_gt_flp):
            data_sample = DetDataSample()
            data_sample.gt_instances = gt_instances

            img_meta = batch_img_metas[j % num_imgs].copy()
            img_meta['crop_size'] = self.crop_size
        ...
        # custom head interface: loss(feat, batch_inputs, data_samples, rot)
        losses = self.bbox_head.loss(
            feat, batch_inputs, batch_data_samples_all, rot)
